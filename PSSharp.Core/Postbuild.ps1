...
$Config['RootModule'] = "$ModuleName.dll"
$Config['ModuleDirectory'] = Join-Path $TargetDirectory -ChildPath $Config['ModuleName']
$Config['PrivateCommands'] = @('Get-MethodReference', 'Test-TypeDataImportPreference', 'Get-CompilerValidTypeName')
...
$Config['FileMap'] = @(
    ...
    @{
		...
		'Collection' = @(
            ...
            @{ 'Value' = $TargetName ; 'DestinationDirectory' = $Config['ModuleDirectory'] }
        )
    }
    @{
		'Priority' = 10;
...
foreach ($targetSource in $Config['ImportFiles']) {
	...
	Get-ChildItem @TargetSource | ForEach-Object {
		...
		$_ # Pipe out of ForEach-Object -> all values piped into a single iteration of Copy-ModuleItem to better utilize the pipeline
	} | Copy-ModuleItem
}
